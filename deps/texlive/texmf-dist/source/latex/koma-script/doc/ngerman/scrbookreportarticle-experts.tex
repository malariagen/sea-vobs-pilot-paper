...
Gruppe ausgeführt werden soll, gibt es hingegen
...
seinem ersten Arguments ignoriert werden, so ist stattdessen
...
\item[\PValue{part}:] \ChangedAt{v3.18}{\Class{scrbook}\and
  ...
  \Macro{add\dots}-Variante. Für die Konfiguration der vorhandenen oder neuer
  ...
  die Sternformen automatisch zusammen mit \DescRef{maincls.cmd.part}
...
\item[\PValue{section}:] Stil\textnote{Gliederungsstil
  ...
  Überschriften in diesem Stil können definiert werden. Für die Konfigurierung
  ...
  zwingend.  \PValue{font}, \PValue{indent} sind dringend empfohlen.
  \PValue{tocindent} und \PValue{tocnumwidth} können abhängig vom Namen der
  Gliederungsebene ebenfalls zwingend sein. Das gilt auch, falls ein Befehl,
  der bisher kein Gliederungsbefehl war, mit \Macro{RedeclareSectionCommand}
  zu einem Gliederungsbefehl umdefiniert wird. Die\textnote{Achtung!}
  Anweisung \Macro{addsec} wird ebenso wie die Sternformen automatisch
  zusammen mit \DescRef{maincls.cmd.section} umkonfiguriert und kann nicht
  unabhängig davon verändert werden. Bei der Definition eines
  Gliederungsbefehls in diesem Stil wird gleichzeitig ein gleichnamiges
  Element angelegt, dessen Schrifteinstellung mit
...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \PValue{style} & \PName{Name} & Legt den Stil der Überschriften fest und
    ...
    \PValue{tocstyle} & \PName{Name} & Legt den Stil der Verzeichniseinträge
                                       ...
                                       \PName{Name} verhindert die Definition
                                       des Befehls \Macro{l@\dots} für die
                                       Verzeichniseinträge.\\
    \ChangedAt[2016/03]{v3.20}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
